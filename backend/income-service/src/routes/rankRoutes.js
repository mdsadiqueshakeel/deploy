...
router.get("/user-summary/:userId", async (req, res) => {
  try {
    ...
    // 1. Fetch all RankReward records for this user
    const userRewards = await RankReward.find({ userId });

    // 2. Create map of claimed trips for quick lookup
    const claimedMap = {};
    userRewards.forEach(r => {
      claimedMap[r.rankAchieved] = {
        status: r.status,
    ...
    // 3. Build detailed rank status list
    const rankList = RANK_TIERS.map(tier => {
      const rewardInfo = claimedMap[tier.title] || null;
      ...
      return {
        ...
        reward: tier.reward,
        unlocked: !!rewardInfo,
        trip: rewardInfo?.trip || "NONE",
        tripClaimed: rewardInfo?.tripClaimed || false,
        status: rewardInfo?.status || "NOT_ELIGIBLE",
        claimedAt: rewardInfo?.claimedAt || null,
      };
    });

    // 4. Calculate trip summary
...
router.patch("/claim-trip/:rewardId", async (req, res) => {
  try {
    ...
    if (reward.reward.tripClaimed) {
      return res.status(400).json({ message: "🚫 Trip already claimed!" });
    }

    // ✅ Otherwise, mark as claimed
